...
public class DungeonGame {
    ...
    public int calculateMinimumHP(int[][] dungeon) {
        ...
        // For last column, calculate the element value for each row
        for (int i = m - 2; i >= 0; i--) {
            health[i][n - 1] = Math.max(health[i + 1][n - 1] - dungeon[i][n - 1], 1);
        }

        // For last row, calculate the element value for each column
        for (int j = n - 2; j >= 0; j--) {
        ...
        for (int i = m - 2; i >= 0; i--) {
            for (int j = n - 2; j >= 0; j--) {
                ...
                // Find the minimum between the down or right
