...
public class FindDuplicateFileInSystem {
    ...
    private void process(String path, Map<String, List<String>> fileMap) {
        String[] dirs = path.split(" ");
        String directory = dirs[0];

        for (int i=1; i<dirs.length; i++) {
            String fileAndContent = dirs[i];

            int start = fileAndContent.indexOf('(');
            int end = fileAndContent.indexOf(')');
            String file = fileAndContent.substring(0, start);
            String content = fileAndContent.substring(fileAndContent.indexOf('('), end);

            fileMap.compute(content, (k,v)-> v == null ? new ArrayList<>() : v).add(directory+"/"+file);
        }
    }
