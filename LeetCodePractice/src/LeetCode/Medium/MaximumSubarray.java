...
public class MaximumSubarray {
     ...
     * maxSubArray for A[i: j].
     * In this way, our goal is to figure out what maxSubArray(A, 0, A.length - 1) is. 
     * However, if we define the format of the sub problem in this way, it's hard to find the connection from the
     * sub problem to the original problem(at least for me).
     ...
     * Note that now the sub problem's format is less flexible and less powerful than the previous one because there's
     * a limitation that A[i] should be contained in that sequence and we have to keep track of each solution of the sub
     * problem to update the global optimal value.
