...
public class MyCalendarI {
    ...
    TreeMap<Integer, Integer> calendars = new TreeMap();

    public MyCalendarI() {}

    /**
     * Time Complexity (Java): O(NlogN),
     * where N is the number of events booked. For each new event, we search that the event is legal in O(logN) time,
    ...
    public boolean book(int start, int end) {
        ...
        if ((prev == null || calendars.get(prev) <= start) && (next == null || end <= next)) {
