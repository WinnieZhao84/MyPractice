...
public class ShoppingOffers {
    ...
    public int shoppingOffers(List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
        return this.shopping(price, special, needs, 0);
    }

    private int shopping(List<Integer> price, List<List<Integer>> special, List<Integer> needs, int index) {
        if (index == special.size()) {
            return this.cal(price, needs);
        }
        List<Integer> clone = new ArrayList<>(needs);
        int j=0;
        for (j=0; j<special.get(index).size()-1; j++) {
            int diff = clone.get(j) - special.get(index).get(j);
            if (diff < 0) {
                break;
            }
            clone.set(j, diff);
        }
        if (j == special.get(index).size()-1) {
            return Math.min(special.get(index).get(j) + this.shopping(price, special, clone, index),
                    this.shopping(price, special, needs, index+1));
        }
        else {
            return this.shopping(price, special, needs, index+1);
        }
    }

    private int cal(List<Integer> price, List<Integer> need) {
        int sum = 0;
